...
function extractCourseCode(
  ...
  for (const tag of tags) {
    // API data may contain null/invalid tag values despite the TypeScript type.
...
export function mapBookToResource(book: BookDto): Resource {
  return {
    ...
    // Pass the real cover through untouched — null stays null.
    // ResourceCard decides how to render the null case (file-type icon),
    // so no placeholder photo is substituted here.
